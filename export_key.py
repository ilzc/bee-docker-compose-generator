...
parser = argparse.ArgumentParser(description='Swarm Bee backup')
parser.add_argument('--clefdir', '-d', help='输入clef keystore文件夹，在文件夹下搜索', required=True, nargs='+')
parser.add_argument('--output', '-o', help='输出csv文件', default="address_key.csv" )
...
with open(args.output, mode='w') as csv_file:
    ...
    for keyfile in args.clefdir:
        for filename in glob.glob(keyfile+'/keystore/UTC*'):
            print(filename)
            if "UTC" in filename: 
                with open(keyfile + '/password', 'r') as stream:
                    password = stream.read()
                    result = extract_key_from_keyfile(filename, password)
                    private_key = bytes(result).hex()
                    address = keys.PrivateKey(result).public_key.to_checksum_address()
                    employee_writer.writerow([address,private_key])                    